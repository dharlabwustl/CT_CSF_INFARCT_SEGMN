...
def measure_NWU_after_subt_csf_Oct_5_2020(): #niftifilename,npyfiledirectory,niftifilenamedir):
    ...
    if os.path.exists(csf_seg_maskbasename_path) and os.path.exists(CSF_Mask_filename) and os.path.exists(niftifilename):
        ...
        lower_thresh=np.min(filename_gray_data_np) #int(float(sys.argv[7])) #20 #0 # 20 #
        upper_thresh=np.max(filename_gray_data_np) #int(float(sys.argv[8])) #80 # 40 # 80 #
        lower_thresh_normal=np.min(filename_gray_data_np) #int(float(sys.argv[7]))
        upper_thresh_normal=np.max(filename_gray_data_np)#int(float(sys.argv[8]))
