...
#  outputfiles_present=0000000000000000000000
  ...
  if [[ "${outputfiles_present: -1}" -eq 0 ]]; then
    ...
    #  echo "It's there."
    #fi
    for file in ${output_dirname}/*levelset*.nii.gz; do

      cp ${file} ${final_output_directory}/

    done

    for file in /software/unet_ich_edema/Results_model_114/*.nii*; do
      if [[ "${file}" != *"ERICH"* ]]; then
        echo "It's there."
    ...
    for file in /software/unet_ich_edema/Results_model_72/*.nii*; do
      if [[ "${file}" != *"ERICH"* ]]; then
        echo "It's there."
